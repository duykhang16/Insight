            ...
            <div className="max-h-[calc(100vh-300px)] overflow-auto custom-scrollbar">
                <table className="w-full min-w-[900px] text-left text-sm whitespace-nowrap">
                    <thead className="text-slate-500 border-b border-white/5 sticky top-0 z-10 th-bg-surface">
                        <tr>
                            <th onClick={() => onSort('name')} className="px-8 py-5 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors min-w-[220px]">
                                Network Name <SortIcon column="name" />
                            </th>
                            <th onClick={() => onSort('vlan')} className="px-6 py-5 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors min-w-[100px]">
                                VLAN <SortIcon column="vlan" />
                            </th>
                            <th onClick={() => onSort('type')} className="px-6 py-5 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors min-w-[120px]">
                                Type <SortIcon column="type" />
                            </th>
                            <th className="px-6 py-5 font-black uppercase tracking-widest text-[10px] min-w-[280px]">
                                Attached SSIDs
                            </th>
                            <th onClick={() => onSort('clients')} className="px-6 py-5 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors text-right min-w-[120px]">
                                Clients <SortIcon column="clients" />
                            </th>
                            <th onClick={() => onSort('status')} className="px-6 py-5 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors min-w-[100px]">
                    ...
                    <tbody className="divide-y divide-white/5 th-text-secondary">
                        ...
                        {loading && Array.from({ length: 6 }).map((_, i) => (
                            <tr key={`skel-${i}`} className="animate-pulse">
                                <td className="px-8 py-5"><div className="h-4 th-bg-elevated rounded w-44" /></td>
                                ...
                                <td className="px-6 py-5"><div className="h-4 th-bg-elevated rounded w-56" /></td>
                                <td className="px-6 py-5 text-right"><div className="h-4 th-bg-elevated rounded w-12 ml-auto" /></td>
