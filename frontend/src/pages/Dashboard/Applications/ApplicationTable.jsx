            ...
            <div className="max-h-[calc(100vh-250px)] overflow-auto custom-scrollbar">
                <table className="w-full min-w-[800px] text-left text-sm whitespace-nowrap block md:table">
                    <thead className="text-slate-500 border-b border-white/5 table-header-sticky">
                        <tr>
                            <th onClick={() => onSort('name')} className="px-8 py-6 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors group table-col-sticky table-header-sticky resizable-col min-w-[250px]">
                                Category <SortIcon column="name" />
                            </th>
                            <th onClick={() => onSort('usage')} className="px-8 py-6 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors group text-right resizable-col min-w-[150px]">
                                24h Usage <SortIcon column="usage" />
                            </th>
                            <th onClick={() => onSort('percentage')} className="px-8 py-6 font-black uppercase tracking-widest text-[10px] cursor-pointer hover:th-text-primary transition-colors group text-right resizable-col min-w-[150px]">
