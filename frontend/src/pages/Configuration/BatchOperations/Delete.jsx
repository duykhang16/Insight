                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="backdrop-blur-2xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-3xl p-6 flex flex-col gap-4 shadow-xl dark:shadow-none min-h-[500px]">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">{t('batch_delete.execution_log')}</h3>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[400px] space-y-2 pr-1 custom-scrollbar">
                            {logs.length === 0 ? (
                                ...
                                logs.map((log) => (
                                    <div
                                        key={log.id}
                                        className={`flex items-start gap-3 p-3.5 rounded-xl text-xs border transition-colors ${log.status === 'ok'
                                            ? 'bg-emerald-50/70 dark:bg-emerald-500/[0.06] border-emerald-200 dark:border-emerald-500/10'
                                            : log.status === 'error'
                                                ? 'bg-rose-50/70 dark:bg-rose-500/[0.06] border-rose-200 dark:border-rose-500/10'
                                                : log.status === 'running'
                                                    ? 'bg-rose-50/50 dark:bg-rose-500/[0.04] border-rose-200 dark:border-rose-500/10 animate-pulse'
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className={`font-bold ${log.status === 'error' ? 'text-rose-700 dark:text-rose-400' : 'text-slate-700 dark:text-slate-300'} text-[11px] break-all leading-relaxed`}>
                                                {log.msg}
                                            </p>
                                        </div>
                                    </div>
