...
import { SUBNET_MASK_OPTIONS, DEFAULT_INTERNAL_SUBNET_MASK, calculateDhcpRange } from '../constants';
...
const WirelessIpAssignment = ({
    ...
    const dhcpRange = isSpecificToThisNetwork
        ? calculateDhcpRange(form.networkAddress, form.subnetMask)
        : null;

    return (
        <div className="rounded-[2rem] border border-white/10 bg-slate-900/90 p-6 md:p-8 shadow-2xl">
            <div className={`grid gap-10 ${isSpecificToThisNetwork ? 'xl:grid-cols-[1fr,0.95fr]' : ''}`}>
                    ...
                    {isSpecificToThisNetwork ? (
                        <div className="space-y-5">
                            ...
                            <LabeledField label="Subnet Mask">
                                <select
                                    value={SUBNET_MASK_OPTIONS.some((option) => option.value === form.subnetMask) ? form.subnetMask : DEFAULT_INTERNAL_SUBNET_MASK}
                            ...
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="space-y-5 text-sm">
                                    <div>
                                        ...
                                        <div className="mt-2 font-semibold text-white">{dhcpRange?.startIp || '—'}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">End IP Address</div>
                                        <div className="mt-2 font-semibold text-white">{dhcpRange?.endIp || '—'}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Available IP Addresses</div>
                                        <div className="mt-2 font-semibold text-white">{dhcpRange ? String(dhcpRange.available) : '—'}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
